...
cpp::result<std::tuple<DF<HAR_X>,DF<HAR_Y>>,std::string> read_valid()
{
    return df::read_bz2<HAR_X,HAR_Y,1>(valid_file.c_str());
}
...
void train_and_save(const cxxopts::ParseResult& options)
{
    ...
    auto m_df = har::read_train_and_valid();
    if (m_df.has_error())
        Util::die("{}", m_df.error());
    ...
    if (args.algo == TrainingAlgo::Robust)
    {
        ...
        auto m_atkr = har::new_Attacker(args.budget, X, args.feature_ids, args.epsilon);
        if (m_atkr.has_error())
...
void batch_train_and_save(const cxxopts::ParseResult& options, const std::string& batch_file)
{
    ...
    auto attackers = parse_batch_file<HAR_X>(batch_file, args.attack_file, args.budget, args.epsilon);
...
void argument_sweep(const cxxopts::ParseResult& options)
{
                          ...
                          sweep_index++)
    {
        ...
        if (arg.algo == TrainingAlgo::Robust)
        {
            ...
            auto m_atkr = har::new_Attacker(arg.budget, X, arg.feature_ids, arg.epsilon);
            if (m_atkr.has_error())
                Util::die("{}", m_atkr.error());
