...
const std::filesystem::path data_dir = "data/forest/";
    ...
    size_t min_instances_per_node, bool affine)
{
    ...
    auto m_atkr = forest::new_Attacker(50, X);
    if (m_atkr.has_error())
    ...
    auto tree = forest::new_RDT({.id = 0, .attacker = std::move(m_atkr.value()),
